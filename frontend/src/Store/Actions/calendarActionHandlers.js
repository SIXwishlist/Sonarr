...
function getDates(time, view, firstDayOfWeek) {
...
const calendarActionHandlers = {
  [types.FETCH_CALENDAR]: function(payload) {
    return function(dispatch, getState) {
      // TODO: Set start/end after fetching/verifying date range is already populated
      ...
      const dates = getDates(moment(time), view, state.settings.ui.item.firstDayOfWeek);
  ...
  [types.GOTO_CALENDAR_PREVIOUS_RANGE]: function(payload) {
    return function(dispatch, getState) {
      const state = getState();
      const view = state.calendar.view;
      const time = moment(state.calendar.time).subtract(1, viewRanges[view]);
  ...
  [types.GOTO_CALENDAR_NEXT_RANGE]: function(payload) {
    return function(dispatch, getState) {
      const state = getState();
      const view = state.calendar.view;
      const time = moment(state.calendar.time).add(1, viewRanges[view]);
